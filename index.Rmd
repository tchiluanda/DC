...
tema <- function(){
    ...
    theme(
      ...
      plot.subtitle = element_text(color = "grey20", face = "plain", size = 8),
...
Bom, vivemos em uma cidade, que tem uma prefeitura, certo? Essa cidade está dentro de um estado, que tem um governo. E isso tudo (incluindo você) fica em um país. Você teria curiosidade em saber o quanto eles [declaram]{.conceito #conceito-declaram} que ~~nós devemos~~ devem?
...
## Origem e Abrangência das informações
...
ggplot(DCLs_entes, aes(y = DC, x = reorder(Escopo, DC), fill = Escopo, color = Escopo)) +
  ...
  geom_text(aes(y = DC + 1e11, label = paste0(
          ...
          " bi")), family = "Varela Round", hjust = "left", size = 3.5) +
  ...
  labs(x = NULL, y = "R$ bi", title = "Dívida Consolidada dos níveis de Governo",
       subtitle = "O Estado de Goiás e alguns municípios não estão incluídos \n(ver \"Abrangência\")") +
  coord_flip() + 
  tema()
...
graf_abrang_m <- ggplot(abrangencia_m, aes(y = Valor, x = Variavel, fill = categorias)) +
  ...
  geom_text(aes(label = percent(percent), y = percent), family = "Varela Round", color = "white", position = position_stack(vjust = 0.5), size = 3) +
...
graf_abrang_e <- ggplot(abrangencia_e, aes(y = Valor, x = Variavel, fill = categorias)) +
  geom_col(position = "fill", width=0.6, color = "white", size = 1) +
  geom_text(aes(label = ifelse(Variavel == "qde", Valor, percent(percent)), y = percent), family = "Varela Round", color = "#1E4C7A", position = position_stack(vjust = 0.5), size = 3) +
...
sumario_estados <- tabela_estados %>%
  ...
  select(nomes = UF, REGIAO, Conta_red, pop, Valor) %>%
...
plota_DC_estados <- function(dados) {
  ggplot(dados, 
    ...
    geom_text(
      aes(
        label = paste0(
          ...
          " bi"),
        y = DC/1e9 + 1), 
      family = "Varela Round", size = 3, color = azul, vjust = 0.3, hjust = 'left') +
    ...
    labs(y = "R$ bilhões", x = NULL, title = "Dívida Consolidada dos Estados em 2019",
         caption = "Goiás não havia informado seus dados até abril de 2020.",
         fill = "Região") +
...
ranking_estados <- sumario_estados %>%
  mutate_at(vars(c(-UF, -pop, -REGIAO, -nomes)), .funs = ~dense_rank(-.)) %>%
  gather(-UF, -pop, -REGIAO, -nomes, key = Variavel, value = Valor) %>%
...
ggplot(dados_composicoes, aes(y = valor/100, x = UF, fill = variavel)) +
  ...
  geom_text(aes(label = percent(valor/100, accuracy = .1)),
            family = "Varela Round", size = 3, hjust = "inward", 
...
ggplot(total_DCL, aes(y = UF, color = Escopo, x = DCL_percent)) +
  ...
  geom_text(aes(x = ifelse(Estado_Maior,
                           ...
                           ifelse(Escopo == "Estados",
                                  DCL_percent - 0.15,
                                  DCL_percent + 0.18)),
                label = percent(DCL_percent)), family = "Varela Round", size = 3) +
  annotate(geom = "text", x = 2.01, y = 16, label = "Limite definido \npelo Senado Federal \npara os Estados",
           family = "Varela Round", color = tres_cores[1], size = 2.5, hjust = "left") +
  annotate(geom = "text", x = 1.21, y = 16, label = "Limite definido \npelo Senado Federal \npara os Municípios",
           family = "Varela Round", color = tres_cores[2], size = 2.5, hjust = "left") +
