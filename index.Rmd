...
@import url('https://fonts.googleapis.com/css2?family=Merriweather+Sans:ital,wght@0,300;0,400;0,700;0,800;1,400&family=Merriweather:ital,wght@0,300;0,400;0,700;0,900;1,400&display=swap');
...
fonte_pri <- "Merriweather Sans"
fonte_sec <- "Merriweather Sans"

tema <- function(){
    theme_minimal() +
    theme(
      text = element_text(family = fonte_sec, colour = "grey20"),
      title = element_text(family = fonte_pri, face = "bold", size = 10, color = "#1E4C7A"), 
      plot.subtitle = element_text(color = "grey20", face = "plain", size = 9),
...
sumario_municipios_d3 <- sumario_municipios %>%
  select(-categorias, -qde, -posicoes, -Instituicao) %>%
  mutate(DCL = if_else(DCL<0, 0, DCL)) %>%
  group_by(UF) %>%
  mutate(DCL_estado = mean(DCL)) %>%
  ungroup() %>%
  group_by(REGIAO) %>%
  mutate(DCL_regiao = mean(DCL)) %>%
  ungroup()

sumario_estados_d3 <- sumario_estados %>%
  mutate(REGIAO = str_sub(REGIAO, 5))

dados_d3 <- list("estados" = sumario_estados_d3, "municipios" = sumario_municipios_d3)

# tive que fazer essa função, com a opção dataframe = "rows", para evitar que o json importado no chunk de D3 ficasse naquele formato
# {"coluna1" : [array de valores],
#  "coluna2" : [array de valores]}

data_to_json <- function(data) {
  jsonlite::toJSON(data, dataframe = "rows", auto_unbox = FALSE, rownames = TRUE)
}

dados_d3_ <- data_to_json(dados_d3)
...
<select name="menu-estado" id="menu-estado">
  <option value="">--Escolha o Estado--</option>
</select>

<select name="menu-municipio" id="menu-municipio">
    <option value="">--Escolha--</option>
    <option value="dados-estado">Governo Estadual</option>
</select>
...
```{d3 data=dados_d3_, height=240}

// funções úteis para o resto do código
const localeBrasil = {
  "decimal": ",",
  "thousands": ".",
  "grouping": [3],
...
svg.append("text").classed("aviso", true).attr("x", 10).attr("y", 30).attr("font-family", "Merriweather Sans").style("font-size", "12px").text("Faça uma seleção para visualizar o gráfico!")
...
function draw_card(UF, opcao_municipio) {
    ...
    let bars_enter = bars.enter().append("rect").classed("barras", true);
    
    bars = bars.merge(bars_enter);
    ...
    let axisText_enter = axisText.enter().append("p").classed("axis-text", true);
    
    axisText = axisText.merge(axisText_enter);
    ...
    let labels = svg//d3.select("div.container-d3")
        .selectAll("text.labels")
        .data(categorias, d => d.tipo);
        
    let labels_enter = labels.enter().append("text").classed("labels", true);
    
    labels.exit().remove();
    
    labels = labels.merge(labels_enter);
    ...
    labels
        ...
        .text(d => "R$ "+valor_formatado(mini_dados[d.tipo]))
        .attr("y", function(d) {
            const altura = d3.select(this).node().getBoundingClientRect().height;
            return y(d.tipo) + (bar_height/2 - altura/2) + altura
        }); 
}
