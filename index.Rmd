...
function draw_card(UF, opcao_municipio) {
  ...
  let max_valor = Math.max(
    d3.max(mini_dados, d => d.Ded),
    d3.max(mini_dados, d => d.DC),
    d3.max(mini_dados, d => d.DCL)
  );
  ...
  let y = d3.scaleBand()
            .domain(["DC", "Ded", "DCL"])
            .range([margin.top, margin.top + 300]);
            
  let bars = svg
    .selectAll("rect")
    .data(mini_dados, d => d.UF)
    .join("rect")
    .classed("barras")
    .attr("x", d => x)
...
//svg.append("rect").attr("x", 50).attr("y", 50).attr("width", 20).attr("height", 20);
svg.append("text").attr("x", 50).attr("y", 100).text(UF_selecionada + ", e mun: " + opcao_mun_selecionada + " e width, height: " + width + " x " + height)
