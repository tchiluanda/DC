...
<p style = "text-align: center; font-size: 0.7em;">Créditos da imagem: William Leite, http://www.willtirando.com.br/</p>
...
Vivemos em uma cidade, que tem uma prefeitura, certo? Essa cidade está dentro de um estado, que tem um governo. E isso tudo ~~incluindo você~~ fica em um país. Você teria curiosidade em saber o quanto nós devemos eles <span class="conceito">declaram</span> que devem?

<div class="descricao">

<h3>Que história é essa de declarar?</h3>
...
ggplot(abrangencia_m, aes(y = Valor, x = Variavel, fill = categorias)) +
  ...
  scale_fill_manual(values = c("RGF" = tres_cores[2], "faltantes" = "#02dba9"), 
                    labels = c("faltantes" = "Não entregues", "RGF" = "Entregues")) +
...
ggplot(abrangencia_e, aes(y = Valor, x = Variavel, fill = categorias)) +
  geom_col(position = "fill", width=0.6, color = "white", size = 1) +
  geom_label(aes(label = percent(percent), y = percent), family = "Open Sans", color = "white", position = position_stack(vjust = 0.5), size = 3) +
...
output$DCLperCap <- renderText(paste0("DCL / RCL: ",
   format(round(as.numeric(retorna_texto(obtem_estado_click(), "pct_DCL_RCL"))/100,2),
                                       big.mark = ".", decimal.mark = ",")))
...
sumario_municipios <- tabela_municipios %>%
  ...
  select(REGIAO, Instituicao, Valor, Conta, pop, UF, Escopo) %>%
  spread(Conta, Valor) %>%
  filter(`Instituicao` != "Prefeitura Municipal de Itaíba - PE")


a <- ggplot(sumario_municipios, aes(y = pct_DCL_RCL, x = REGIAO, color = pct_DCL_RCL > 0, label = Instituicao)) +
