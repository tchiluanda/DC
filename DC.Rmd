...
#     gather(ded, dc, key = variavel, value = valor)
# }
# 
# sumario_grafico("DF")
...
faz_mapa <- function() {
  leaflet(mapa_brasil, options = leafletOptions(minZoom = 4, maxZoom = 4)) %>%
    ...
    addPolygons(
      ...
      labelOptions = labelOptions(
        ...
        textsize = "15px",
        direction = "auto")) %>% 
    addLegend(pal = pal, values = ~percent(DCL/100), opacity = 0.7, title = NULL,
    position = "bottomleft")
}
...
output$Estado    <- renderText(paste0("Estado: ", retorna_texto(input$mapa_shape_click$id, "nomes")))
output$regiao    <- renderText(retorna_texto(input$mapa_shape_click$id, "REGIAO"))
output$pop       <- renderText(paste0("População: ",
                                      format(
                                        round(
                                          as.numeric(
                                            retorna_texto(input$mapa_shape_click$id, "pop")
                                            ),
                                          2),
                                        big.mark = ".", decimal.mark = ","), 
                                      " pessoas"))
output$DCLperCap <- renderText(paste0("DCL por habitante: R$ ",
   format(round(as.numeric(retorna_texto(input$mapa_shape_click$id, "DCL_pop")),2),
                                       big.mark = ".", decimal.mark = ",")))
