...
plota_card_estado <- function(dados) {
  ggplot(dados, aes(x = variavel, y = valor, fill = cores)) + 
    ...
    geom_text(aes(y = valor_media, 
                  ...
                  vjust = ifelse(medias == "Média Nacional", 3.2, -2.2)), family = "Open Sans Condensed", size = 3,
              hjust = "inward") +
    ...
    scale_x_discrete(labels = c("ded" = "Deduções", "dc" = "DC", "dcl" = "DCL")) +
    labs(x = NULL, y = "R$ bilhões") +
    coord_flip() +
    tema() + theme(axis.text.y = element_text(size = 12))
}
...
output$card <- renderPlot(height = 300,
